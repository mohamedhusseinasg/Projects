...
class ProductTemplate(models.Model):
            ...
            if len(self.bom_ids.ids) > 1:
                ...
                for bom in [self.bom_ids[0]]:
                    ...
                    for bom_line in bom.bom_line_ids.filtered(lambda bl: not bl.product_id.is_packaging):
                        bom_ingridients = bom_line.product_id.product_tmpl_id._recursive_bom_ingredients(
                            qty=bom_line.product_qty, uom=bom_line.product_uom_id, level=level,
                            ingredients=bom_ingridients)
                ...
                for bom_line in bom.bom_line_ids.filtered(lambda bl: not bl.product_id.is_packaging):
                    bom_ingridients = bom_line.product_id.product_tmpl_id._recursive_bom_ingredients(
                        qty=bom_line.product_qty, uom=bom_line.product_uom_id, level=level, ingredients=bom_ingridients)
                self.write_nutrition_facts(bom_ingridients, qty=bom.product_qty, uom=bom.product_uom_id)
    ...
    def add_to_ingridients_list(self, ingredients, qty=0, uom=False):
        ...
        _logger.debug("Facts: %s %s (%s kcal) of %s", qty, self.uom_id.name, self.energy_calories * multiplier,
                      self.display_name)
    ...
    def write_nutrition_facts(self, ingredients, qty=0, uom=False):
        ...
        energy_joule, energy_calories, fat_total, fat_saturated, carbohydrate, carbo_sugars, roughage, protein, sodium = [
            0, 0, 0, 0, 0, 0, 0, 0, 0]
