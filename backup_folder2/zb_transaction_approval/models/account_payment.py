...
class AccountPayment(models.Model):
    ...
    def action_approval(self):
        """
        This method sends an approval email for the current payment to all users in the 'group_payment_approver' group. 
        sets the 'approval_send' field of the payment to True.
        """
        template = self.env.ref('zb_transaction_approval.mail_template_payment_approval')
        for order in self:
            if order.approval_send == True:
                raise ValidationError(_("Document is already send for approval. Please refresh the page."))
            else:
                if template:
                    approver_group = self.env.ref('zb_transaction_approval.group_payment_approver') 
                    if approver_group:
                        ...
                        order.message_post(body="Approval email sent for this payment.")
